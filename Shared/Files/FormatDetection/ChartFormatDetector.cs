...
namespace DJMaxEditor.Files.FormatDetection
{
    ...
    public static class ChartFormatDetector
    {
        ...
        public static FormatDetectionResult Detect(byte[] data, string extensionHint = null)
        {
            ...
            // (4) bmson - a JSON object (so it can't be any binary signature) carrying the v1
            //     sound-channel tables. Checked before PTFF because the orders can't collide:
            //     a '{' at offset 0 is never "PTFF".
            string bmsonEvidence;
            if (LooksLikeBmson(data, out bmsonEvidence))
            {
                return new FormatDetectionResult(ChartFormat.Bmson, DetectionConfidence.High,
        ...
        /// <summary>
        /// bmson v1 is UTF-8 JSON: an object holding <c>sound_channels</c> arrays of notes. The
        /// marker is checked textually over the first megabyte rather than by parsing, because
        /// detection only inspects — it must never throw for malformed input, and a half-written
        /// JSON file that still carries the marker is exactly the case a "Probable" would paper
        /// over. Files written before the v1 tables (which used <c>bmson_notes</c>) are accepted
        /// too: they are rare but real, and the parser downstream rejects what is genuinely
        /// unreadable.
        /// </summary>
        private static bool LooksLikeBmson(byte[] d, out string evidence)
        {
            evidence = null;
            int i = 0;
            ...
            string head = System.Text.Encoding.ASCII.GetString(d, i, limit - i);
            if (head.IndexOf("\"sound_channels\"", StringComparison.Ordinal) >= 0)
            {
                evidence = "JSON object with bmson \"sound_channels\"";
                return true;
            }
