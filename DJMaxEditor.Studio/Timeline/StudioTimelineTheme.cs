...
namespace DJMaxEditor.Studio.Timeline
{
    ...
    internal sealed class StudioTimelineTheme
    {
        ...
        private static StudioTimelineTheme _default;

        private readonly Dictionary<TechnikaNoteKind, NoteBrushes> _noteBrushes =
            new Dictionary<TechnikaNoteKind, NoteBrushes>();

        private StudioTimelineTheme()
        {
            Field = StudioPalette.Brush(StudioPalette.CanvasField);
            FieldDead = StudioPalette.Brush(StudioPalette.CanvasFieldDead);
            FieldBackground = StudioPalette.Brush(StudioPalette.CanvasFieldBackground);
            ...
            GridSub = FrozenPen(StudioPalette.GridSub, HairlineWidth);
            GridBeat = FrozenPen(StudioPalette.GridBeat, HairlineWidth);
            GridBar = FrozenPen(StudioPalette.GridBar, HairlineWidth);
            LaneEdge = FrozenPen(StudioPalette.LaneEdge, HairlineWidth);
            ...
            LaneEdgeStrong = FrozenPen(StudioPalette.LaneEdgeStrong, 2.0);
            ChromeEdgePen = FrozenPen(StudioPalette.Edge, HairlineWidth);

            Playhead = FrozenPen(StudioPalette.Accent, 1.5);
            PlayheadGlow = Translucent(StudioPalette.Accent, 0.16);

            MarqueeFill = Translucent(StudioPalette.Accent, 0.14);
            MarqueeEdge = FrozenPen(StudioPalette.Accent, HairlineWidth);

            HoverFill = Translucent(StudioPalette.TextPrimary, 0.06);
            SelectionEdge = FrozenPen(StudioPalette.NoteSelectedEdge, 1.5);
            ...
            TextOnNote = StudioPalette.Brush("#FFF2F5FA");
        ...
        public static StudioTimelineTheme Default
        {
            get
            {
                if (_default == null)
                {
                    _default = new StudioTimelineTheme();
                }
                return _default;
            }
        }
        ...
        private void BuildNoteBrushes()
        {
            NoteBrushes selected = new NoteBrushes(
                StudioPalette.NoteSelected, StudioPalette.NoteSelectedEdge);
            NoteBrushes basic = new NoteBrushes(
                StudioPalette.NotePlayable, StudioPalette.NotePlayableEdge);
            NoteBrushes accent = new NoteBrushes(
                StudioPalette.NoteAccent, StudioPalette.NoteAccentEdge);
            NoteBrushes hold = new NoteBrushes(
                StudioPalette.NoteLong, StudioPalette.NoteLongEdge);
            ...
            BgaBrushes = new NoteBrushes(StudioPalette.NoteBga, StudioPalette.NoteBgaEdge);
            BgaBrushes.Selected = selected;
            BackgroundBrushes = new NoteBrushes(
                StudioPalette.NoteBackground, StudioPalette.NoteBackgroundEdge);
            BackgroundBrushes.Selected = selected;

            _chips = new Brush[StudioPalette.TrackChips.Length];
            for (int i = 0; i < StudioPalette.TrackChips.Length; i++)
            {
                _chips[i] = StudioPalette.Brush(StudioPalette.TrackChips[i]);
            }
        }
